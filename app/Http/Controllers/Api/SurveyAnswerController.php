...
class SurveyAnswerController extends Controller
{
    ...
    public function getPublicDoneSurvey(Request $request)
    {
        ...
        $SurveyAnswer = SurveyAnswer::where('voucher_code', $search)
            ->get()
            ->first();
    ...
    public function getSurveyAnswer(Request $request, $id, $security_code, $receipt_number)
    {
        ...
        if (!$FormHistory) {
            ...
            return GlobalFunction::invalid(
                Message::SURVEY_ANSWER_ALREADY_DONE,
                [
    ...
    public function checkEntryCode(RegisterCheckingRequest $request, $mobile_number, $receipt_number, $store_id)
    {
        ...
        if ($today->between(Carbon::parse($SurveyPeriod->valid_from), Carbon::parse($SurveyPeriod->valid_to))) {
            ...
            if ($receiptNumberByStore->expiration_date <= Carbon::now()->format('Y-m-d H:i:s')) {
                return GlobalFunction::invalid(
            ...
            if ($receiptNumberByStore->is_done) {
                $VoucherId = SurveyAnswer::where('receipt_number', $receipt_number)
                    ->where('mobile_number', $mobile_number)
                    ->where('store_id', $store_id)
                    ->latest()
                    ->first();
    ...
    public function createSurvey(SurveyAnswerRequest $request)
    {
        ...
        try {
            ...
            // return Carbon::now();
            $today = Carbon::now()->format('Y-m-d H:i:s');
            $valid_to = Carbon::parse($SurveyPeriod->valid_to);
            $valid_from = Carbon::parse($SurveyPeriod->valid_from);
            ...
            // Check if the mobile number exists
            $receiptNumberByMobile = ReceiptNumber::where('contact_details', $request["mobile_number"])
                ->where('is_valid', 1)
                ->where('is_valid', 1)
                ->latest()
                ->first();
            ...
            $receiptNumberByReceipt = ReceiptNumber::where('contact_details', $request["mobile_number"])
            ...
            // Check if the store ID exists for the provided mobile number and receipt number
            $receiptNumberByStore = ReceiptNumber::where('contact_details', $request["mobile_number"])
                ->where('receipt_number', $request["receipt_number"])
            ...
            if ($receiptNumberByStore->expiration_date > Carbon::now()->format('Y-m-d H:i:s') && $receiptNumberByStore->is_done == true) {
                return GlobalFunction::invalid("You have already used your available voucher.");
            }
            ...
            // Add receipt_number from CreateSurveyAnswer
            $responseData['receipt_number'] = $CreateSurveyAnswer->receipt_number;
    ...
    public function updateSurveyAnswer(Request $request, $id)
    {
        ...
        try {
            ...
            $SurveyAnswerId = SurveyAnswer::withTrashed()
                ->where('id', $id)
                ->first();
            ...
            foreach ($questionnaire_answers as $questionnaire_answer) {
                ...
                foreach ($questionnaire_answer['questions'] as $question) {
                    if (!isset($question['questionType'])) {
                        continue; // Skip if questionType is not set
                    }
                    ...
                    if ($question['questionType'] === 'grid') {
                        foreach ($question['answer'] as $gridAnswer) {
                            ...
                            $finalAnswer = $gridAnswer['rowAnswer'] === "Other"
                                ? (is_array($gridAnswer['otherAnswer'] ?? null) ? implode(', ', $gridAnswer['otherAnswer']) : ($gridAnswer['otherAnswer'] ?? ''))
                        ...
                        foreach ($answers as $answer) {
                            ...
                            $finalAnswer = $answer === "Other"
                                ? (is_array($question['otherAnswer'] ?? null) ? implode(', ', $question['otherAnswer']) : ($question['otherAnswer'] ?? ''))
                                : $answer;
            ...
            $receiptNumber = ReceiptNumber::where('contact_details', $SurveyAnswerId->mobile_number)
    ...
    public function claimingVoucher(Request $request, $id)
    {
        ...
        //add additional voucher check if the user has a tag store
        if (auth('sanctum')->user()->user_type != "cashier") {
            return GlobalFunction::invalid("Cannot claim voucher: Cashier permissions are required for this action.");
        }
        ...
        return GlobalFunction::response_function(
            Message::VOUCHER_CLAIM_SUCCESSFULLY,
            [
                "voucher_code" => $SurveyAnswerId->voucher_code,
    ...
    public function extendVoucher(ExtendValidityRequest $request)
    {
        ...
        if ($extendDate > $SurveyPeriod->valid_to) {
            $finaldate = $SurveyPeriod->valid_to;
        } else {
            $finaldate = $extendDate;
        }
    ...
    public function getDataChart(Request $request)
    {
        ...
        $ChartData = SurveyAnswer::when($status === "inactive", function ($query) {
            $query->onlyTrashed();
        })
